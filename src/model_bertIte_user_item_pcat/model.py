...
class BertIte(nn.Module):
    def __init__(self, config, user_reprs, item_reprs, dim_repr):
        super().__init__()
        ...
        user_reprs = torch.FloatTensor(user_reprs)
        item_reprs.append([0] * dim_repr)
        item_reprs = torch.FloatTensor(item_reprs)

        self.embedding_user_repr = nn.Embedding.from_pretrained(user_reprs, freeze=True)
        self.emb_rep_u = nn.Linear(self.dim_repr, self.num_factor, bias=False)
    ...
    def forward(self, uids, item_sequences, target_items, attention_masks):
        t = time.time()
        user_reps = self.embedding_user_repr(uids)
        self.user_reps_emb = self.emb_rep_u(user_reps)
        self.uids_emb = self.embedding_user(uids)
        self.uids_embedding = self.uids_emb * 0.5 + self.user_reps_emb * 0.5 
    ...
    def compute_loss(self, click, action, implicit_labels, explicit_labels):
        ...
        regularizer = torch.add(torch.mean(torch.square(self.uids_emb)), torch.mean(torch.square(self.items_sequences)))
...
class Manager:
    ...
    def train_and_evaluate(self):
        ...
        model = BertIte(cf, du.user_representation, du.item_representation, du.dimension)
        ...
        for epoch_i in range(epoch_ii+1, epochs):
            ...
            if (epoch_i + 1) % 10 == 0:
                path = f'./checkpoints_test/{num_factor}_{cf.batch_size}_{cf.eta}_{lr}'
                torch.save({
                    'epoch': epoch_i,
